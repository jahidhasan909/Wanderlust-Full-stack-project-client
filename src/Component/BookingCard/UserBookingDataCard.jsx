                ...
                <div className="flex flex-1 flex-col gap-3">
                    <Card.Header className="gap-1 flex flex-row justify-between items-center p-2">
                        <div className=" space-y-2">
                            <Card.Title className="font-bold text-2xl">{bookingData.destinationName}</Card.Title>

                            <div className=" space-y-1">
                                <p className="text-xs text-zinc-500">Departure : {new Date(bookingData.departureDate).toLocaleDateString(
                                    new Intl.DateTimeFormat('en-US', {
                                        dateStyle: 'long'
                                    })
                                )}</p>
                                <p className="text-xs text-zinc-500">Booking ID : {bookingData._id}</p>
                            </div>


                            <h2 className="font-semibold text-cyan-500 text-2xl">${bookingData.price}</h2>
